...
%%%-------------------------------------------------------------------
-module(emqx_backend_dolphindb_actions).
-author("root").
...
-export([on_action_create_data_to_dolphindb/2,on_action_destroy_data_to_dolphindb/2]).
-export([on_action_data_to_dolphindb/2]).
...
-resource_type(#{create => on_resource_create,
  description =>
  #{en =>
  <<68, 111, 108, 112, 104, 105, 110, 68, 66, 32, 68,
    97, 116, 97, 98, 97, 115, 101>>,
    zh =>
    <<68, 111, 108, 112, 104, 105, 110, 68, 66, 32, 230,
      149, 176, 230, 141, 174, 229, 186, 147>>},
  destroy => on_resource_destroy,
  name => backend_dolphindb,
  params =>
  #{auto_reconnect =>
  #{default => true,
    description =>
    ...
    <<73, 102, 32, 114, 101, 45, 116, 114,
      ...
      115, 116>>,
      zh =>
      <<68, 111, 108, 112, 104, 105, 110, 68,
        66, 32, 232, 191, 158, 230, 142, 165,
    ...
    #{default => <<>>,
      ...
      <<80, 97, 115, 115, 119, 111, 114, 100,
        ...
        116, 111, 32, 68, 111, 108, 112, 104,
        105, 110, 68, 66>>,
        zh =>
        <<68, 111, 108, 112, 104, 105, 110, 68,
          66, 32, 229, 175, 134, 231, 160,
          129>>},
      order => 4,
      title =>
      #{en =>
      <<68, 111, 108, 112, 104, 105, 110, 68,
        66, 32, 80, 97, 115, 115, 119, 111,
        114, 100>>,
        zh =>
        <<68, 111, 108, 112, 104, 105, 110, 68,
          66, 32, 229, 175, 134, 231, 160,
    ...
    server =>
    #{default =>
    <<49, 50, 55, 46, 48, 46, 48, 46, 49, 58, 56,
      56, 52, 56>>,
      description =>
      #{en =>
      <<68, 111, 108, 112, 104, 105, 110, 68,
        66, 32, 73, 80, 32, 97, 100, 100, 114,
        101, 115, 115, 32, 111, 114, 32, 104,
        111, 115, 116, 110, 97, 109, 101, 32,
        97, 110, 100, 32, 112, 111, 114, 116>>,
        zh =>
        <<68, 111, 108, 112, 104, 105, 110, 68,
          66, 32, 230, 156, 141, 229, 138, 161,
    ...
    <<85, 115, 101, 114, 110, 97, 109, 101,
      ...
      zh =>
      <<68, 111, 108, 112, 104, 105, 110, 68,
        66, 32, 231, 148, 168, 230, 136, 183,
        229, 144, 141>>},
      order => 3, required => true,
      title =>
      #{en =>
      <<68, 111, 108, 112, 104, 105, 110, 68,
        66, 32, 85, 115, 101, 114, 32, 78, 97,
        109, 101>>,
        zh =>
        <<68, 111, 108, 112, 104, 105, 110, 68,
          66, 32, 231, 148, 168, 230, 136, 183,
  ...
  title =>
  #{en => <<68, 111, 108, 112, 104, 105, 110, 68, 66>>,
    zh => <<68, 111, 108, 112, 104, 105, 110, 68, 66>>}}).

-rule_action(#{category => data_persist,
  create => on_action_create_data_to_dolphindb,
  description =>
  #{en =>
  <<68, 97, 116, 97, 32, 116, 111, 32, 68, 111, 108, 112,
    104, 105, 110, 68, 66>>,
    zh =>
    <<228, 191, 157, 229, 173, 152, 230, 149, 176, 230,
      141, 174, 229, 136, 176, 32, 68, 111, 108, 112, 104,
      105, 110, 68, 66, 32, 230, 149, 176, 230, 141, 174,
      229, 186, 147>>},
  destroy => on_action_destroy_data_to_dolphindb,
  for => '$any', name => data_to_dolphindb,
  params =>
  #{'$resource' =>
  #{description =>
  #{en =>
  <<66, 105, 110, 100, 32, 97, 32, 114, 101,
    ...
    <<83, 81, 76, 32, 116, 101, 109, 112, 108,
      97, 116, 101, 115, 32, 119, 105, 116,
      104, 32, 112, 108, 97, 99, 101, 104, 111,
      108, 100, 101, 114, 115, 32, 102, 111,
      114, 32, 105, 110, 115, 101, 114, 116,
      105, 110, 103, 32, 100, 97, 116, 97, 32,
      105, 110, 116, 111, 32, 97, 32, 68, 111,
      108, 112, 104, 105, 110, 68, 66, 32, 100,
      97, 116, 97, 98, 97, 115, 101, 46>>,
      zh =>
      <<229, 140, 133, 229, 144, 171, 228, 186,
        ...
        230, 143, 146, 229, 133, 165, 230, 149,
        176, 230, 141, 174, 229, 136, 176, 32,
        68, 111, 108, 112, 104, 105, 110, 68, 66,
  ...
  <<68, 97, 116, 97, 32, 116, 111, 32, 68, 111, 108, 112,
    ...
    <<228, 191, 157, 229, 173, 152, 230, 149, 176, 230,
      141, 174, 229, 136, 176, 32, 68, 111, 108, 112, 104,
      105, 110, 68, 66>>},
  types => [backend_dolphindb]}).


on_resource_create(ResId,#{<<"server">> := Server, <<"username">> := Username, <<"password">> := Password} = Opts) ->
  begin
    logger:log(info,
      #{},
      #{report_cb =>
  ...
  {Host, Port} = parse_host(Server),
  Options = [{host, Host},
    {port, Port},
    {username, Username},
    {password, Password},
    {auto_reconnect,
      case maps:get(<<"auto_reconnect">>, Opts, true) of
        true -> 15;
        false -> false
      end},
    {pool_size, maps:get(<<"pool_size">>, Opts, 8)}],
  PoolName = list_to_atom("dolphindb:" ++ str(ResId)),
  ...
  case test_resource_status(PoolName) of
    true -> ok;
    false ->
      error({{backend_dolphindb, ResId}, connection_failed})
  end,
  #{<<"pool">> => PoolName}.
...
test_resource_status(PoolName) ->
  Status = [begin
              case ecpool_worker:client(Worker) of
                {ok, Conn} ->
                  ok == element(1, dolphinc:run(Conn, <<"">>));
                _ -> false
              end
            end
...
-spec on_action_create_data_to_dolphindb(ActId :: binary(), map()) -> fun((Msg :: map()) -> any()).
on_action_create_data_to_dolphindb(ActId,
    Opts = #{<<"pool">> := PoolName,
      <<"sql">> := SQL,
      <<"enable_batch">> := true}) ->
  BatcherPName = list_to_atom("dolphindb_batcher:" ++
  str(ActId)),
...
on_action_create_data_to_dolphindb(ActId,
    Opts = #{<<"pool">> := PoolName,
      <<"sql">> := SQL}) ->
  SQLTks = emqx_rule_utils:preproc_tmpl(SQL),
  {[{'Opts', Opts},
    {'PoolName', PoolName},
    {'SQL', SQL},
    {'ActId', ActId},
...
on_action_data_to_dolphindb(Selected,
    _Envs = #{'__bindings__' :=
    #{'ActId' := ActId,
      'BatcherPName' := BatcherPName,
      'SQLParamPartTks' :=
      ...
      'SyncTimeout' := SyncTimeout}}) ->
  SQLInsertPart =
    emqx_rule_utils:proc_tmpl(SQLInsertPartTks, Selected),
  SQLParamPart =
    emqx_rule_utils:proc_tmpl(SQLParamPartTks, Selected),
  begin
    logger:log(debug,
      #{},
      #{report_cb =>
      fun (_) ->
        {logger_header() ++
          "DolphinDB Pool: ~p, Selected: ~p",
          [BatcherPName, {SQLInsertPart, SQLParamPart}]}
      end,
        mfa =>
        {emqx_backend_dolphindb_actions,
          on_action_data_to_dolphindb,
          2},
        line => 295})
  end,
...
on_action_data_to_dolphindb(Selected,
    ...
    #{'ActId' := ActId,
      'SQLTks' := SQLTks,
      'PoolName' := PoolName}}) ->
  SQL = emqx_rule_utils:proc_tmpl(SQLTks, Selected),
  begin
    logger:log(debug,
      #{},
      #{report_cb =>
  ...
  case ecpool:pick_and_do(PoolName,
    {emqx_backend_dolphindb_actions, insert, [SQL]},
    no_handover)
  of
    {ok, _, _} ->
      emqx_rule_metrics:inc_actions_success(ActId),
      begin
        logger:log(debug,
          ...
          fun (_) ->
            {logger_header() ++
              "dolphindb_run_sql successfully",
              []}
          end,
            mfa =>
            {emqx_backend_dolphindb_actions,
...
on_action_destroy_data_to_dolphindb(_ActId,
    #{batcher_pname := BatcherPName}) ->
  emqx_rule_actions_utils:stop_batcher_pool(BatcherPName);
on_action_destroy_data_to_dolphindb(_ActId, _) -> ok.

connect(Options) -> dolphinc:start_link(Options).

batcher_flush(Batch, State) ->
  {batch_sql_insert(Batch,
    {emqx_backend_dolphindb_actions,
      batch_insert,
      [State]}),
    State}.
...
batch_sql_insert(Batch, InsertFun) ->
  ...
  lists:flatmap(fun ({SQLInsertPart, FromAndValList}) ->
    {FromList, SQLParamList} =
      lists:unzip(FromAndValList),
    SQL = insert_batch_sql(SQLInsertPart,
      transpose(datalist(SQLParamList))),
    Result = safe_exec(InsertFun,
      [SQL, length(SQLParamList)]),
    [{From, Result} || From <- FromList]
                end,
    AggrBatch).

insert_batch_sql(SQLInsertPart, Datalist) ->
  SQLParamPartStr = values(Datalist),
  [SQLInsertPart, " values (", SQLParamPartStr, ")"].
    ...
    E:R:ST ->
      begin
        logger:log(error,
          ...
          fun (_) ->
            {logger_header() ++
              "safe_exec ~p, failed: ~0p",
              [Action, {E, R, ST}]}
          end,
            mfa =>
            {emqx_backend_dolphindb_actions, safe_exec, 2},
            line => 370})
      end,
  ...
  begin
    logger:log(debug,
      ...
      fun (_) ->
        ...
        mfa =>
        {emqx_backend_dolphindb_actions, async_call_back, 3},
        line => 380})
  end,
...
parse_host(Server) when is_binary(Server) ->
  case string:split(Server, ":", trailing) of
    ...
    [Host] -> {parse_ip(Host), 8848}
  end.

datalist(SQLParamList) ->
  {ok, RE} = re:compile("[(), ]"),
  lists:map(fun (R) ->
    [I || I <- re:split(R, RE), I /= <<>>]
            end,
...
transpose([[] | _]) -> [];
transpose(M) ->
  [lists:map(fun hd/1, M) | transpose(lists:map(fun tl/1,
    M))].

values(Cols) ->
  lists:join(",",
    [begin ["["] ++ lists:join(",", Col) ++ ["]"] end
      || Col <- Cols]).

logger_header() -> "[RuleEngine Dolphin] ".
