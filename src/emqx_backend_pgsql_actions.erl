...
%%%-------------------------------------------------------------------
-module(emqx_backend_pgsql_actions).
-author("root").
...
-include("../include/emqx_backend_pgsql.hrl").
...
-export([on_action_create_data_to_pgsql/2,
  on_action_data_to_pgsql/2,
  on_action_destroy_data_to_pgsql/2]).

-export([on_action_create_data_to_timescale/2,
  on_action_data_to_timescale/2,
  on_action_destroy_data_to_timescale/2]).

-export([on_action_create_offline_msg/2,
  on_action_offline_msg_to_pgsql/2]).

-export([on_action_create_lookup_sub/2,
  on_action_lookup_sub_to_pgsql/2]).

-resource_type(#{create => on_resource_create,
  description =>
  #{en =>
  <<80, 111, 115, 116, 103, 114, 101, 83, 81, 76>>,
    zh => <<80, 111, 115, 116, 103, 114, 101, 83, 81, 76>>},
  destroy => on_resource_destroy, name => backend_pgsql,
  params =>
  #{auto_reconnect =>
  #{default => true,
    description =>
    ...
    <<73, 102, 32, 82, 101, 45, 116, 114,
      ...
      115, 116>>,
      zh =>
      <<232, 191, 158, 230, 142, 165, 230, 150,
        173, 229, 188, 128, 230, 151, 182, 230,
        152, 175, 229, 144, 166, 233, 135, 141,
        232, 191, 158>>},
    order => 5,
    title =>
    #{en =>
...
-resource_type(#{create => on_resource_create,
  description =>
  #{en =>
  <<84, 105, 109, 101, 115, 99, 97, 108, 101, 68, 66>>,
    zh =>
    <<84, 105, 109, 101, 115, 99, 97, 108, 101, 68,
      66>>},
  destroy => on_resource_destroy,
  name => backend_timescale,
  params =>
  #{auto_reconnect =>
  #{default => true,
    description =>
...
-rule_action(#{category => data_persist,
  create => on_action_create_data_to_pgsql,
  description =>
  #{en =>
  <<83, 116, 111, 114, 101, 32, 100, 97, 116, 97, 32,
    116, 111, 32, 80, 111, 115, 116, 103, 114, 101, 83,
    81, 76>>,
    zh =>
    <<228, 191, 157, 229, 173, 152, 230, 149, 176, 230,
      141, 174, 229, 136, 176, 32, 80, 111, 115, 116, 103,
      114, 101, 83, 81, 76>>},
  destroy => on_action_destroy_data_to_pgsql,
  for => '$any', name => data_to_pgsql,
  params =>
  #{'$resource' =>
  #{description =>
  #{en =>
  <<66, 105, 110, 100, 32, 97, 32, 114, 101,
    ...
    <<83, 81, 76, 32, 84, 101, 109, 112, 108,
      ...
      <<229, 140, 133, 229, 144, 171, 228, 186,
        ...
        32, 80, 111, 115, 116, 103, 114, 101, 83,
        81, 76, 32, 230, 149, 176, 230, 141, 174,
        229, 186, 147>>},
      input => textarea, order => 5, required => true,
      title =>
      #{en =>
      <<83, 81, 76, 32, 84, 101, 109, 112, 108,
        97, 116, 101>>,
  ...
  title =>
  #{en =>
  <<68, 97, 116, 97, 32, 116, 111, 32, 80, 111, 115, 116,
    103, 114, 101, 83, 81, 76>>,
    zh =>
    <<228, 191, 157, 229, 173, 152, 230, 149, 176, 230,
      141, 174, 229, 136, 176, 32, 80, 111, 115, 116, 103,
      114, 101, 83, 81, 76>>},
...
-rule_action(#{category => offline_msgs,
  ...
  <<79, 102, 102, 108, 105, 110, 101, 32, 77, 115, 103,
    32, 116, 111, 32, 80, 111, 115, 116, 103, 114, 101,
    83, 81, 76>>,
    zh =>
    <<231, 166, 187, 231, 186, 191, 230, 182, 136, 230,
      129, 175, 228, 191, 157, 229, 173, 152, 229, 136,
      176, 32, 80, 111, 115, 116, 103, 114, 101, 83, 81,
      76>>},
  for => '$any', name => offline_msg_to_pgsql,
  params =>
  #{'$resource' =>
  #{description =>
  #{en =>
...
-rule_action(#{category => server_side_subscription,
  ...
  <<71, 101, 116, 32, 83, 117, 98, 115, 99, 114, 105,
    ...
    70, 114, 111, 109, 32, 80, 111, 115, 116, 103, 114,
    101, 83, 81, 76>>,
    zh =>
    <<228, 187, 142, 32, 80, 111, 115, 116, 103, 114, 101,
      83, 81, 76, 32, 228, 184, 173, 232, 142, 183, 229,
      143, 150, 232, 174, 162, 233, 152, 133, 229, 136,
      151, 232, 161, 168>>},
  for => '$any', name => lookup_sub_to_pgsql,
  params =>
  #{'$resource' =>
  #{description =>
  #{en =>
  ...
  <<83, 117, 98, 115, 99, 114, 105, 112, 116, 105, 111,
    ...
    32, 80, 111, 115, 116, 103, 114, 101, 83, 81, 76>>,
    zh =>
    <<228, 187, 142, 32, 80, 111, 115, 116, 103, 114, 101,
      83, 81, 76, 32, 228, 184, 173, 232, 142, 183, 229,
      143, 150, 232, 174, 162, 233, 152, 133, 229, 136,
...
-rule_action(#{category => data_persist,
  create => on_action_create_data_to_timescale,
  description =>
  #{en =>
  <<83, 116, 111, 114, 101, 32, 68, 97, 116, 97, 32, 116,
    111, 32, 84, 105, 109, 101, 115, 99, 97, 108, 101>>,
    zh =>
    <<228, 191, 157, 229, 173, 152, 230, 149, 176, 230,
      141, 174, 229, 136, 176, 32, 84, 105, 109, 101, 115,
      99, 97, 108, 101>>},
  destroy => on_action_destroy_data_to_timescale,
  for => '$any', name => data_to_timescale,
  params =>
  #{'$resource' =>
  #{description =>
  #{en =>
  <<66, 105, 110, 100, 32, 97, 32, 114, 101,
    ...
    <<83, 81, 76, 32, 84, 101, 109, 112, 108,
      ...
      <<229, 140, 133, 229, 144, 171, 228, 186,
        ...
        150, 230, 155, 180, 230, 150, 176, 230,
        149, 176, 230, 141, 174, 229, 136, 176,
        32, 84, 105, 109, 101, 115, 99, 97, 108,
        101, 32, 230, 149, 176, 230, 141, 174,
        229, 186, 147>>},
  ...
  title =>
  #{en =>
  <<68, 97, 116, 97, 32, 116, 111, 32, 84, 105, 109, 101,
    115, 99, 97, 108, 101>>,
    zh =>
    <<228, 191, 157, 229, 173, 152, 230, 149, 176, 230,
      141, 174, 229, 136, 176, 32, 84, 105, 109, 101, 115,
      99, 97, 108, 101>>},
  types => [backend_timescale]}).
...
on_resource_create(ResId,
  ...
  begin
    logger:log(info,
      ...
      fun (_) ->
        ...
        mfa =>
        {emqx_backend_pgsql_actions, on_resource_create, 2},
        line => 466})
  end,
  {ok, _} = application:ensure_all_started(ecpool),
  {Host, Port} =
    emqx_rule_actions_utils:parse_server(Server, 5432),
  ...
  PoolName = list_to_atom("pgsql:" ++ str(ResId)),
  ...
  begin
    logger:log(info,
      ...
      fun (_) ->
        ...
        mfa =>
        {emqx_backend_pgsql_actions, on_resource_destroy, 2},
        line => 495})
  end,
  emqx_rule_actions_utils:stop_pool(PoolName).

-spec on_action_create_data_to_pgsql(Id :: binary(),
    #{}) -> fun((Msg :: map()) -> any()).

on_action_create_data_to_pgsql(ActId,
  ...
  begin
    logger:log(info,
      ...
      fun (_) ->
        {logger_header() ++
          "Initiating Batch Action ~p, SQL: ~p",
          [on_action_create_data_to_pgsql, SQL]}
      end,
        mfa =>
        {emqx_backend_pgsql_actions,
          on_action_create_data_to_pgsql,
          2},
        line => 500})
  end,
  BatcherPName = list_to_atom("pgsql_batcher:" ++
  str(ActId)),
...
on_action_create_data_to_pgsql(ActId,
  ...
  PrepareSqlKey = binary_to_list(ActId),
  {PrepareStatement, ParamsTokens} =
    emqx_rule_utils:preproc_sql(SQL, '$n'),
...
on_action_data_to_pgsql(Data,
    _Envs = #{'__bindings__' :=
    #{'EnableBatch' := true, 'ActId' := ActId,
      'BatcherPName' := BatcherPName,
      'SQLInsertPartTks' := SQLInsertPartTks,
      'SQLParamPartTks' := SQLParamPartTks,
      'InsertMode' := InsertMode,
  ...
  begin
    logger:log(debug,
      ...
      #{report_cb =>
      fun (_) ->
        {logger_header() ++ "Pool: ~p, Data: ~0p",
          [BatcherPName, AccumData]}
      end,
        mfa =>
        {emqx_backend_pgsql_actions,
          on_action_data_to_pgsql,
          2},
        line => 535})
  end,
...
on_action_data_to_pgsql(Data,
    ...
    #{'ActId' := ActId,
      ...
      'PrepareStatement' := PrepareStatement,
      'ParamsTokens' := ParamsTokens}}) ->
  case pgsql_query(PoolName,
    PrepareSqlKey,
    PrepareStatement,
    emqx_rule_utils:proc_sql(ParamsTokens, Data))
  of
    {error, Reason} ->
      begin
        logger:log(error,
...
on_action_destroy_data_to_pgsql(_ActId,
    #{batcher_pname := BatcherPName}) ->
  emqx_rule_actions_utils:stop_batcher_pool(BatcherPName);
on_action_destroy_data_to_pgsql(_ActId, _) -> ok.

-spec on_action_create_data_to_timescale(Id :: binary(),
    #{}) -> fun((Msg :: map()) -> any()).

on_action_create_data_to_timescale(ActId,
    #{<<"enable_batch">> := EnableBatch = true,
      <<"pool">> := PoolName, <<"sql">> := SQL} =
      Opts) ->
  begin
    logger:log(info,
      ...
      fun (_) ->
        {logger_header() ++
          "Initiating Batch Action ~p, SQL: ~p",
          [on_action_create_data_to_timescale, SQL]}
      end,
        mfa =>
        {emqx_backend_pgsql_actions,
          on_action_create_data_to_timescale,
          2},
        line => 571})
  end,
  BatcherPName = list_to_atom("pgsql_batcher:" ++
...
on_action_data_to_timescale(Data, Envs) ->
  on_action_data_to_pgsql(Data, Envs).

on_action_destroy_data_to_timescale(ActId, Opts) ->
  on_action_destroy_data_to_pgsql(ActId, Opts).

-spec on_action_create_offline_msg(Id :: binary(),
    #{}) -> fun((Msg :: map()) -> any()).
...
on_action_create_offline_msg(ActId,
  ...
  "values ($1, $2, $3, $4, $5, $6, to_timestamp("
  "$7) )">>,
  DeleteSql = <<"delete from mqtt_msg where msgid = $1 "
  "and topic = $2">>,
  SelectSql =
    <<"select topic, msgid, sender, qos, payload, "
    "retain, extract(epoch from arrived) "
    "as arrived from mqtt_msg where topic "
    "= $1 order by id DESC">>,
  InsertKey = "insert_" ++ binary_to_list(ActId),
  DeleteKey = "delete_" ++ binary_to_list(ActId),
  SelectKey = "select_" ++ binary_to_list(ActId),
  ...
  {SelectSql1, Params} = case {TimeRange,
    MaxReturnedCount}
                         of
                           ...
                           {TimeRange, undefined} ->
                             {<<SelectSql/binary,
                               " and arrived >= to_timestamp($2)">>,
                               [TimeRange div 1000]};
                           {undefined, MaxReturnedCount} ->
                             {<<SelectSql/binary, " limit $2">>,
                               [MaxReturnedCount]};
                           {TimeRange, MaxReturnedCount} ->
                             {<<SelectSql/binary,
                               " and arrived >= to_timestamp($2) limit $3">>,
...
on_action_offline_msg_to_pgsql(Msg = #{event := Event,
  topic := Topic},
    _Envs = #{'__bindings__' :=
    #{'ActId' := ActId,
      'PoolName' := PoolName,
...
-spec on_action_create_lookup_sub(Id :: binary(),
    #{}) -> fun((Msg :: map()) -> any()).
...
on_action_lookup_sub_to_pgsql(Msg = #{event := Event,
  clientid := ClientId},
    _Envs = #{'__bindings__' :=
    #{'ActId' := ActId,
      'PoolName' := PoolName,
...
prepare_sql_to_conn(Conn, PrepareList) ->
  lists:foreach(fun ({PrepareSqlKey, PrepareStatement}) ->
    ...
    of
      ...
      {error, sync_required} -> epgsql:sync(Conn);
      {error, Reason} ->
        begin
          logger:log(error,
            #{},
            ...
            fun (_) ->
              {logger_header()
                ++
                "Create Prepare Statement Failed: ~p, "
                "reason: ~0p",
                [PrepareStatement,
                  Reason]}
            end,
        ...
        error({prepare_sql_failed, Reason})
    end
                end,
    PrepareList).
...
insert_message(Pool, ActId, PrepareSqlKey,
  ...
  case pgsql_query(Pool,
    PrepareSqlKey,
    PrepareStatement,
    PrepareParams)
  of
    {ok, _} -> emqx_rule_metrics:inc_actions_success(ActId);
    {error, Error} ->
      begin
        logger:log(error,
          #{},
...
lookup_message(Pool, ActId, PrepareSqlKey,
  ...
  of
    ...
    {ok, Cols, Rows} ->
      emqx_rule_metrics:inc_actions_success(ActId),
      [begin
         Msg = emqx_backend_pgsql_cli:r2m(Cols, Row),
...
connect(Options) -> epgsql:connect(Options).

pgsql_query(PoolName, PrepareStatementId,
    PrepareStatement, PrepareParams) ->
  begin
    logger:log(debug,
      #{},
      ...
      fun (_) ->
        {logger_header() ++
          "Query Pool: ~p, PrepareStatementId: "
          "~p, SqlParams: ~p",
          [PoolName, PrepareStatementId, PrepareParams]}
      end,
        mfa => {emqx_backend_pgsql_actions, pgsql_query, 4},
        line => 797})
  end,
  ecpool:pick_and_do(PoolName,
    {emqx_backend_pgsql_actions,
      do_pgsql_query,
      [PrepareStatementId, PrepareStatement, PrepareParams]},
    no_handover).

do_pgsql_query(Conn, PrepareStatementId,
    PrepareStatement, PrepareParams) ->
  case epgsql:prepared_query(Conn,
    PrepareStatementId,
    PrepareParams)
  of
    {error,
      #error{codename = invalid_sql_statement_name}} ->
      prepare_sql_to_conn(Conn,
        [{PrepareStatementId, PrepareStatement}]),
      epgsql:prepared_query(Conn,
        PrepareStatementId,
        PrepareParams);
    Res -> Res
  end.
...
squery(Conn, SQL) -> epgsql:squery(Conn, SQL).
  ...
  begin
    logger:log(error,
      ...
      fun (_) ->
        ...
        mfa => {emqx_backend_pgsql_actions, async_call_back, 3},
        line => 822})
  end,
...
batch_insert(SQL, _BatchLen,
  ...
  ecpool:pick_and_do(PoolName,
    {emqx_backend_pgsql_actions, squery, [SQL]},
